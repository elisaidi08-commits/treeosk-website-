...
const top = await p.evaluate(()=>document.querySelector("#instore")?.offsetTop||0);
for (const [dy,name] of [[700,"is-zoom"],[1500,"is-reveal"]]){
  await p.evaluate(y=>window.scrollTo(0,y), top+dy);
  await p.waitForTimeout(1100); await p.screenshot({path:`${base}/${name}.png`});
}
