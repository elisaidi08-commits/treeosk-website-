...
await p.waitForTimeout(1500); await p.screenshot({path:`${base}/v-clients.png`});
// curtain between instore and how
const how=await p.evaluate(()=>document.querySelector("#how")?.offsetTop||0);
await p.evaluate(y=>window.scrollTo(0,y-500), how);
await p.waitForTimeout(1400); await p.screenshot({path:`${base}/v-curtain.png`});
