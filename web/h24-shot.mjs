...
for (const [id,name] of [["#experiences","tabs-exp"],["#system","tabs-cap"]]){
  await p.evaluate(s=>{const e=document.querySelector(s); if(e) window.scrollTo(0, e.offsetTop-40);}, id);
  await p.waitForTimeout(1600); await p.screenshot({path:`${base}/${name}.png`});
}
